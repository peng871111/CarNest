...
function getStatusTone(status: VehicleOperationalStatus) {
...
function StatusPill({ label }: { label: VehicleOperationalStatus }) {
...
export function VehicleManagementHub({
  ...
  const customerRows = useMemo(() => {
    return [...customerProfiles]
      ...
      .map((profile) => {
        const linkedRecords = vehicleRecords
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <h4 className="mt-1 text-sm font-semibold text-ink">{vehicleRecord ? getVehicleRecordTitle(vehicleRecord) : intake.vehicleTitle || "Warehouse intake"}</h4>
                    ...
                    <div className="text-right text-xs text-ink/55">
                      <p className="font-semibold text-ink">{intake.status.replace(/_/g, " ")}</p>
        ...
        <section className="space-y-6">
          <div className="rounded-[28px] border border-black/5 bg-white p-6 shadow-panel">
            ...
            <div className="mt-5 grid gap-4 xl:grid-cols-2">
              {listingRelationshipRows.slice(0, 12).map(({ vehicle, linkedRecord, linkedCustomer }) => (
                <div key={vehicle.id} className="rounded-[22px] border border-black/6 bg-shell px-4 py-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <StatusPill label={getListingOperationalStatus(vehicle) || "Listed"} />
                  </div>
